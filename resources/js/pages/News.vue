...
const highlightData = computed(() => {
    if (props.highlightedResult) {
        return {
            ...
            suggestion: props.highlightedResult.suggestion ?? null,
        };
    }
    return null;
});
...
<template>
    ...
    <div class="bg-muted-foreground min-h-screen w-full px-4 py-6">
        <div class="mx-auto max-w-4xl">
            ...
            <div class="flex justify-between space-x-4">
                <h1 class="text-primary mb-6 text-3xl font-bold drop-shadow-md">{{ title?.length == 1 ? title[0] : title }}</h1>
    ...
    <Dialog v-model:open="showDialog">
        <DialogContent class="min-w-2xl">
            ...
            <DialogFooter class="items-center space-x-4">
                <p class="inline-flex items-center">
                    {{ highlightData.rank === 1 ? "Congratulations! don't forget to claim your prize" : 'Almost there! Try again next month' }}
                </p>
                <Button variant="outline" size="sm" @click="showDialog = false" class="ml-auto">Close</Button>
            </DialogFooter>
        </DialogContent>
    </Dialog>
</template>
